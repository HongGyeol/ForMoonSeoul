...
<body>
	...
	<main class="container-fluid px-0 mb-5" id="main">
		...
		<div class="main_div">
			<div class="ct_div">
			 	<a href="/t_list.traveldo?ct_u=${ct_u}&ct_d=0" class="${ct_d==0? 'ct_active' : 'ct_detail'}">진행중</a>
			 	<a href="/t_list.traveldo?ct_u=${ct_u}&ct_d=1" class="${ct_d==1? 'ct_active' : 'ct_detail'}">진행예정</a>
			 	<a href="/t_list.traveldo?ct_u=${ct_u}&ct_d=2" class="${ct_d==2? 'ct_active' : 'ct_detail'}">진행완료</a>
			</div>
			...
			<div class="collapse" id="searchPeriod">
				<div class="container text-center" >
					<form name="dateForm" class="form-control" action="/t_list.traveldo" method="post">
						...
						<input type="datetime-local" class="searchKeyword ddate" name="date_start" value="${date_start ne ''? searchDate_s:''}">
						 
						<label class="label">~ 종료일 </label>
		   				<input type="datetime-local" class="searchKeyword ddate" name="date_end" value="${date_end ne ''? searchDate_e:''}">
			...
			<div class="collapse" id="searchKeyword">
		    	<div class="container text-center">
		    		<form name="keywordForm" class="form-control" antion="/t_list.traveldo" method="post" >
		    			...
		    			<input type="text" class="searchKeyword" name="searchKeyword" placeholder="키워드" value="${searchKeyword ne ''? searchKeyword:''}">
			...
			<c:if test="${count>0}">
					...
					<div class="mb">
						...
						<a href="/t_detail.traveldo?num=${vo.getNum()}&ct_u=${ct_u}&ct_d=${ct_d}&searchKeyword=${searchKeyword}&date_start=${date_start}&date_end=${date_end}&pageNum=${currentPage}">
							<div class="mb_board">  
		...
		<c:if test="${count>0}">
			...
			<c:if test="${firstPage ne startPage}">
				<a href="/t_list.traveldo?ct_u=${ct_u}&ct_d=${ct_d}&pageNum=${firstPage}&searchKeyword=${sechKeyword eq ''? '':searchKeyword}&date_start=${date_start eq ''? '':date_start}&date_end=${date_end eq ''? '':date_end}" class="page_f"> << </a>
				         
				<a href="/t_list.traveldo?ct_u=${ct_u}&ct_d=${ct_d}&pageNum=${startPage-1}&searchKeyword=${sechKeyword eq ''? '':searchKeyword}&date_start=${date_start eq ''? '':date_start}&date_end=${date_end eq ''? '':date_end}" class="page_f"> < </a>
			</c:if>
			
			<c:forEach var="i" begin="${startPage}" end="${endPage}" step="1">
			
			 	<a href="/t_list.traveldo?ct_u=${ct_u}&ct_d=${ct_d}&pageNum=${i}&searchKeyword=${searchKeyword eq ''? '':searchKeyword}&date_start=${date_start eq ''? '':date_start}&date_end=${date_end ne ''? date_end:''}" class="${i eq currentPage? 'page_f_on':'page_f'}">${i}</a>
			
			</c:forEach>
			
			<c:if test="${endPage ne lastPage}">
				<a href="/t_list.traveldo?ct_u=${ct_u}&ct_d=${ct_d}&pageNum=${endPage+1}&searchKeyword=${searchKeyword ne ''? searchKeyword:''}&date_start=${date_start ne ''? date_start:''}&date_end=${date_end ne ''? date_end:''}" class="page_f"> > </a>
					     
				<a href="/t_list.traveldo?ct_u=${ct_u}&ct_d=${ct_d}&pageNum=${lastPage}&searchKeyword=${searchKeyword ne ''? searchKeyword:''}&date_start=${date_start ne ''? date_start:''}&date_end=${date_end ne ''? date_end:''}" class="page_f"> >> </a>
			</c:if>
